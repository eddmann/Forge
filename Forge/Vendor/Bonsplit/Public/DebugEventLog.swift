...
public final class DebugEventLog: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "bonsplit.debug-event-log")
    ...
    private static func resolveLogPath() -> String {
        ...
        if let explicit = env["BONSPLIT_DEBUG_LOG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !explicit.isEmpty {
            return explicit
        }

        if let tag = env["BONSPLIT_TAG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !tag.isEmpty {
            return "/tmp/bonsplit-debug-\(sanitizePathToken(tag)).log"
        }

        if let socketPath = env["BONSPLIT_SOCKET_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !socketPath.isEmpty {
            let socketBase = URL(fileURLWithPath: socketPath).deletingPathExtension().lastPathComponent
            if socketBase.hasPrefix("bonsplit-debug-") {
                return "/tmp/\(socketBase).log"
            }
        }

        if let bundleId = Bundle.main.bundleIdentifier {
            return "/tmp/bonsplit-debug-\(sanitizePathToken(bundleId)).log"
        }

        return "/tmp/bonsplit-debug.log"
    }
