...
struct ProjectListView: View {
    ...
    private func deleteWorkspace(_ workspace: Workspace) {
        ...
        if hasRunning {
            ToastManager.shared.showModal("Stopping processes…")
        } else {
            ToastManager.shared.showModal("Deleting workspace '\(name)'…")
        }
        ProcessManager.shared.clear()
        DispatchQueue.global(qos: .userInitiated).async {
...
private struct ProjectSection: View {
    ...
    private var isProjectActive: Bool {
        ProjectStore.shared.activeProjectID == project.id && activeWorkspaceID == nil
    }
